<template>
	<view>
		<u-navbar :custom-back="goBack" title="资格证书" :titleBold="true" :background="{ background: '#e02e24' }"
			title-color="#fff" back-icon-color="#fff" z-index="99999"></u-navbar>
		<view class="main" v-if="isDis == false">
			<view class="idcardz">
				<view class="idcarc-title">实习律师资格证封二页及首页</view>
				<view class="cardbox">
					<view class="card">
						<template v-if="system == 'android'">
							<view class="slot-btn" @click="uploadPopup(1)" v-if="!readOnly">
								<u-image width="600rpx" border-radius="10" height="347rpx"
									:src="cert1_image +'?x-oss-process=image/resize,m_lfit,w_200,h_200'"
									v-if="cert1_image != '' && cert1_image != undefined"></u-image>
								<u-image width="600rpx" height="347rpx" src="../../../static/img/zeng1.png" v-else>
								</u-image>
							</view>
						</template>
						<template v-else>
			...
			<view class="idcardz">
				<view class="idcarc-title">实习律师资格证第二页、第三页</view>
				<view class="cardbox">
					<view class="card">
						<template v-if="system == 'android'">
							<view class="slot-btn" @click="uploadPopup(2)" v-if="!readOnly">
								<u-image width="600rpx" border-radius="10" height="347rpx"
									:src="cert2_image +'?x-oss-process=image/resize,m_lfit,w_200,h_200'"
									v-if="cert2_image != '' && cert2_image != undefined"></u-image>
								<u-image width="600rpx" height="347rpx" src="../../../static/img/zeng2.png" v-else>
								</u-image>
							</view>
						</template>
						<template v-else>
				...
				<view class="cardbox">
					<view class="card">
						<view class="slot-btn" hover-class="slot-btn__hover" hover-stay-time="150">
							<u-image width="600rpx" height="347rpx"
								:src="cert1_image +'?x-oss-process=image/resize,m_lfit,w_200,h_200'"
								v-if="cert1_image != '' && cert1_image != undefined"></u-image>
...
<script>
	...
	export default {
		...
		methods: {
			...
			onRemove(type) {
				var fawuData = uni.getStorageSync('fawuFrom'+this.userid);
				if (type == 1) {
					this.cert1_image = '';
					fawuData.cert1_image = '';
					fawuData.cert_image = '';
					uni.setStorageSync('fawuFrom'+this.userid, fawuData);
				} else if (type == 2) {
					this.cert2_image = '';
					fawuData.cert2_image = '';
					fawuData.cert_image = '';
					uni.setStorageSync('fawuFrom'+this.userid, fawuData);
				}
			},
			...
			getCard() {
				...
				if (uni.getStorageSync('fawuFrom'+this.userid)) {
					var image = uni.getStorageSync('fawuFrom'+this.userid);
			...
			submit() {
				if (this.cert1_image == '' || this.cert1_image == undefined) {
					...
					fawuData.cert1_image = this.cert1_image;
					fawuData.cert2_image = this.cert2_image;
					fawuData.cert_image = '已上传';
					uni.setStorageSync('fawuFrom'+this.userid, fawuData);
			...
			uploadPopup(e) {
				console.log(e);
				this.thisphoto = e
				// this.cert1_image  = 'https://oss.bigchun.com//10/vv.jpg'
