					...
					<view class="card" >
						<template v-if="system == 'android'">
							<view  class="slot-btn" @click="uploadPopup(1)">
								<u-image width="600rpx" border-radius="10" height="347rpx" :src="cert1_image+'?x-oss-process=image/resize,m_lfit,w_200,h_200'" v-if="cert1_image != '' && cert1_image != undefined"></u-image>
								<u-image width="600rpx" border-radius="10" height="347rpx" src="../../../static/img/zeng6.png" v-else></u-image>
							</view>
							</template>
							<template v-else>
								<u-upload
					...
					<view class="card" >
						<template v-if="system == 'android'">
							<view  class="slot-btn"  @click="uploadPopup(2)">
								<u-image width="600rpx" border-radius="10" height="347rpx" :src="cert2_image+'?x-oss-process=image/resize,m_lfit,w_200,h_200'" v-if="cert2_image != '' && cert2_image != undefined"></u-image>
								<u-image width="600rpx" border-radius="10" height="347rpx" src="../../../static/img/zeng5.png" v-else></u-image>
							</view>
							</template>
							<template v-else>
								<u-upload
...
export default {
	...
	onShow() {
		...
		if ( JSON.stringify(uni.getStorageSync('lawyerFrom'+user_id)) != '{}') {
			...
			if(data){
				if(data.cardface_image){
					this.cert1_image = data.cardface_image;
				}
				if(data.cardback_image){
					this.cert2_image = data.cardback_image;
				}
			}
		}
	},
	...
	methods: {
		...
		onRemove(type) {
			...
			if (type == 1) {
				this.cert1_image = '';
				lawyerData.cardface_image = '';
				lawyerData.card_image = '';
				uni.setStorageSync('lawyerFrom'+this.userid, lawyerData);
			} else if (type == 2) {
				this.cert2_image = '';
				lawyerData.cardback_image = '';
				lawyerData.card_image = '';
				uni.setStorageSync('lawyerFrom'+this.userid, lawyerData);
			}
		},
		...
		getCard() {
			...
			if (uni.getStorageSync('lawyerFrom'+this.userid)) {
				...
				this.cert1_image = image.cardface_image;
				this.cert2_image = image.cardback_image;
			} else {
				this.cert1_image = '';
				this.cert2_image = '';
			}
		...
		submit() {
			if (this.cert1_image == '' || this.cert1_image == undefined) {
				...
				lawyerData.cardface_image = this.cert1_image;
				lawyerData.cardback_image = this.cert2_image;
				lawyerData.card_image = '已上传';
				uni.setStorageSync('lawyerFrom'+this.userid, lawyerData);
