<template>
	<view class="content">
		<u-navbar :custom-back="back" title="上传案例" :background="{background:'#e02e24'}" title-color="#fff" back-icon-color="#fff" z-index="99999">
				</u-navbar>
		<view class="step-box">
			<view class="step-item">
				<view class="item-txt">第一步：添加案例标题</view>
				<u-input class="ipt-border" border border-color="#999" :clearable="false"  type="text" placeholder="请输入" v-model="title" />
			</view>
			<view class="step-item">
				<view class="item-txt">第二步:选择上传领域</view>
				<view class="select" @click="selectLing">
					<text>{{areaname ==''?'请选择':areaname}}</text>
					<u-icon size="40" color="#999" name="arrow-right"></u-icon>
				</view>
				<!-- <u-input class="ipt-border" disabled placeholder="请选择" v-model="title" type="select" /> -->
			</view>
			<view class="step-item" @click="pickPhoto">
				<view class="item-txt">
					<view class="txt-title">第三步：上传图片版</view>
...
export default {
	data() {
		return {
			...
			lingyushow:false,
			readOnly: false,
			isShowUpFile: false, //IOS 不能上传文件，不显示此按钮
			title: '',
			deleteType: 0,
			currentIndex: -1,
			lingyuList:[],
		...
		async toUpload(isOut) {
			...
			if(this.areaname ==''){
				...
				if (isOut) {
					let formData = {
						token: uni.getStorageSync('token'),
						case_images: JSON.stringify(this.source),
						title: this.title,
						fali_serve_area_id:this.areaid
					};
					let res = await this.$api('index.savacase', formData);
					if (res.code == 1) {
						uni.showToast({
							title: res.msg,
							icon: 'none'
